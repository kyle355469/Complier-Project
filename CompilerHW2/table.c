    ...
    while(temp != NULL){
        ...
        for(int i = 0; i < temp->t.tableSize; i++){
            if(strlen(temp->t.id[i].name) > 7){
                printf("%d->\t\t%s\t%s\t", i, temp->t.id[i].name, type_name[temp->t.id[i].type.type]);
            }else{
                printf("%d->\t\t%s\t\t%s\t", i, temp->t.id[i].name, type_name[temp->t.id[i].type.type]);
            }
        ...
        if(temp->innerLink != NULL){
            ...
            for(int i = 0; i < temp->innerLink->t.tableSize; i++){
                if(strlen(temp->innerLink->t.id[i].name) > 7){
                    printf("%s%d->\t\t%s\t%s\t", "ref:",i, temp->innerLink->t.id[i].name, type_name[temp->innerLink->t.id[i].type.type]);
                }else{
                    printf("%s%d->\t\t%s\t\t%s\t", "ref:",i, temp->innerLink->t.id[i].name, type_name[temp->innerLink->t.id[i].type.type]);
                }
...
LT* findTable(LT* tail, char* tableName){
    if(tail != NULL){
        ...
        if(tail->tableName[i+1] == '\0' && tableName[i+1] == '\0'){
            return tail;
        }
    }
    return NULL;
